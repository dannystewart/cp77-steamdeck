...
function flashlight:new()
    registerForEvent("onInit", function()
        ...
        Observe("PlayerPuppet", "OnAction", function(_, action)
            if not self.settings.controllerMode or self.logic.callState == questPhoneCallPhase.IncomingCall then return end

            local actionName = Game.NameToString(action:GetName(action))
            local actionType = action:GetType(action).value

            if actionName == "PhoneInteract" then
                if actionType == "BUTTON_PRESSED" then
                    self.Cron.After(0.2, function()
                        if not self.runtimeData.holding then
